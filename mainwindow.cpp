...
void MainWindow::createMenu() {
    ...
    QAction *userbalanceAct = beginMenu->addAction("清理连接", this,
                                                   &MainWindow::clearNatsInfo);
    userbalanceAct->setStatusTip("清理NATS连接配置信息");
    beginMenu->addSeparator();
    ...
    //工具菜单
    QMenu *toolsMenu = menuBar()->addMenu("信息查询");

    QAction *indepInfoAct = toolsMenu->addAction("信控信息", this,
                                                 &MainWindow::indepInfo);
    indepInfoAct->setStatusTip("查看用户信控信息");
    toolsMenu->addSeparator();

    QAction *slowspeedAct = toolsMenu->addAction("降速信息", this,
                                                 &MainWindow::slowSpeedInfo);
    slowspeedAct->setStatusTip("查看用户达量降速信息");
    toolsMenu->addSeparator();
    toolsMenu->addSeparator();
    ...
    toolBar->addSeparator();
    toolBar->addAction(slowspeedAct);
    toolBar->addSeparator();
    toolBar->addAction(indepInfoAct);
    toolBar->addSeparator();
    ...
    indepInfoAct->setIcon(QIcon(ICON_102));
    userbalanceAct->setIcon(QIcon(ICON_103));
    slowspeedAct->setIcon(QIcon(ICON_101));
    setnatsAct->setIcon(QIcon(ICON_104));
}

void MainWindow::indepInfo() {
    indepInfoTabWidget = new IndepInfoTabWidget();
    setCentralWidget(indepInfoTabWidget);
}

void MainWindow::slowSpeedInfo() {
    slowSpeedInfoTabWidget = new SlowSpeedInfoTabWidget();
    setCentralWidget(slowSpeedInfoTabWidget);
}
