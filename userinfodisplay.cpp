...
UserInfoDisplay::UserInfoDisplay(int flag, QWidget *parent):
    ...
    if(flag == User_Order_Display) {
        ...
        hBox1->addWidget(m_cbOrgId);
        hBox1->addStretch(3);
    } else if(flag == Host_Cmd_Display) {
        hBox1->addWidget(new QLabel("主机选择:"),1);
        hBox1->addWidget(m_cbOrgId,4);
        hBox1->addStretch(1);
        hBox1->addWidget(m_lbServId = new QLabel("执行目录:"), 1);
        hBox1->addWidget(m_leServId = new QLineEdit, 6);
        hBox1->addStretch(6);
    ...
    if(flag == User_Order_Display) {
        ...
        hBox1->addStretch(1);
        hBox1->addWidget(m_pbReSet = new QPushButton("重置"), 1);
        hBox1->addStretch(1);
        hBox1->addWidget(m_pbClear = new QPushButton("清屏"), 1);
    } else {
    ...
    if(flag != Host_Cmd_Display) {
        ...
        vBox->addWidget(m_tbDisplay,8);
        vBox->addWidget(m_pTextEdit,3);
        getPath(m_sAcctID);
        m_leServId->setText(m_sAcctID);
        connect(m_pbReSet, SIGNAL(clicked()), this, SLOT(onReSet()));
        connect(m_pbClear, SIGNAL(clicked()), this, SLOT(onClear()));
    }
    ...
    } else if(m_iFlag == Host_Cmd_Display) {
        hBox1->addStretch(1);
        setStatusTip("主机执行命令");
...
void UserInfoDisplay::onQuery() {
    ...
    if(m_iFlag != Host_Cmd_Display) {
        ...
        m_sAccNbr = m_cbOrgId->currentText();
        if(m_sAccNbr.isEmpty()) {
            QMessageBox::about(this, "提示", "发送主机不可为空");
            return;
        }
        m_sQueryTime = QDateTime::currentDateTime().toString("yyyyMMddhhmmss");
        m_sRes1 = m_pTextEdit->toPlainText().trimmed();
        if(m_sRes1.isEmpty()) {
            QMessageBox::about(this, "提示", "发送命令不可为空");
            return;
        }
        if(!m_sServID.isEmpty()) {
            m_sRes1 = QString("cd %1;%2").arg(m_sServID).arg(m_sRes1);
        }
        setPath(m_sServID);

        m_pTextEdit->clear();
        m_tbDisplay->append(QString("Send:%1\n").arg(m_sRes1));
        if(!Host.contains(m_sAccNbr)) {
            Host.push_back(m_sAccNbr);
            setHost(Host);
        }
    }
    ...
    if(m_iFlag == User_Order_Display) {
        ...
        m_sMsgInfo =
                QString("{\"QueryFlag\":\"%1\",\"ReplySubject\":\"%2\",\"RequestTime\":\"%3\","
                        "\"AccNbr\":\"%5\",\"Res1\":\"%6\"}").
                arg(m_iFlag).arg(m_sNatsRecvTopic).arg(m_sQueryTime).arg(m_sAccNbr).
                arg(m_sRes1);
    } else {
        m_sMsgInfo =
                QString("{\"QueryFlag\":\"%1\",\"ReplySubject\":\"%2\",\"RequestTime\":\"%3\","
...
void UserInfoDisplay::setDisPlay() {
    if(m_bJsonDisplay) {
        ...
        m_tbDisplay->insertPlainText(m_sMsg);
    }
}
...
void UserInfoDisplay::setPath(QString sCmdPath) {
    if(m_sAcctID == sCmdPath)
        return;
    QString sPath = QCoreApplication::applicationDirPath() + "/" + IniFileName;
    QSettings settings(sPath, QSettings::IniFormat);
    settings.setIniCodec("UTF-8");
    settings.setValue("cmdPath", sCmdPath);
    m_sAcctID = sCmdPath;
}

void UserInfoDisplay::getPath(QString &sCmdPath) {
    QString sPath = QCoreApplication::applicationDirPath() + "/" + IniFileName;
    QSettings settings(sPath, QSettings::IniFormat);
    settings.setIniCodec("UTF-8");
    sCmdPath = settings.value("cmdPath").toString();
}

void UserInfoDisplay::setHost(QVector<QString> &Host) {
    QString sPath = QCoreApplication::applicationDirPath() + "/" + IniFileName;
    QSettings settings(sPath, QSettings::IniFormat);
    settings.setIniCodec("UTF-8");
    QString lastLable = settings.value("natsLastLable").toString();
    settings.remove(QString("%1_Host").arg(lastLable));
    settings.beginWriteArray(QString("%1_Host").arg(lastLable));
    int i = 0;
    foreach(QString value, Host) {
        settings.setArrayIndex(i);
        i++;
...
void UserInfoDisplay::getHost(QVector<QString> &Host) {
    // 初始化配置
    QString sPath = QCoreApplication::applicationDirPath() + "/" + IniFileName;
    QSettings settings(sPath, QSettings::IniFormat);
    settings.setIniCodec("UTF-8");
    QString lastLable = settings.value("natsLastLable").toString();
    int size = settings.beginReadArray(QString("%1_Host").arg(lastLable));
    for(int i = 0; i < size; ++i) {
        settings.setArrayIndex(i);
        Host.push_back(settings.value("host").toString());
    }
    settings.endArray();
}
...
bool UserInfoDisplay::eventFilter(QObject *target, QEvent *event)
{
