...
SetNatsInfo::SetNatsInfo(QWidget *parent) :
    ...
    QLabel *labelSendTopic = new QLabel("主  题 ");
    ...
    grid->addWidget(labelSendTopic, 4, 0);
    grid->addWidget(labelLableName, 5, 0);

    grid->addWidget(username, 0, 1);
    grid->addWidget(password, 1, 1);
    grid->addWidget(port, 2, 1);
    grid->addWidget(addr, 3, 1);
    grid->addWidget(sendTopic, 4, 1);
    grid->addWidget(lablename, 5, 1);
    ...
    for(int i = 0; i < size; ++i) {
        ...
        natsInfo.sSendTopic = settings.value("natsSendTopic").toString().trimmed();
    ...
    [ = ](int index) {
        ...
        sendTopic->setText(settings.value("natsSendTopic").toString());
...
void SetNatsInfo::onOK() {
    ...
    natsInfo.sSendTopic = sendTopic->text().trimmed();
    ...
    foreach(NatsInfo value, natsInfoMap) {
        ...
        settings.setValue("natsSendTopic", value.sSendTopic);
